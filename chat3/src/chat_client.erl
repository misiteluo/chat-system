...
init([]) ->
  ...
  % 连接服务器
  case gen_tcp:connect("localhost", 12345, [binary, {packet, 0}, {active, true}]) of
    {ok, Socket} ->
    ...
    {error, Reason} ->
      wxTextCtrl:appendText(ChatDisplay, io_lib:format("连接服务器失败: ~p\n", [Reason])),
      {ok, #state{socket = undefined, username = undefined, frame = Frame, username_input = UsernameInput, chat_display = ChatDisplay, message_input = MessageInput}}
  end.
